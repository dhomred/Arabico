...
class Show365Provider : MainAPI() {
    override var mainUrl = "https://365show.online"
    override var name = "365_SHOW"
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        // أفلام جديدة
        val newMovies = Jsoup.connect("$mainUrl/movies").get()
            .select(".movie-item").map { element ->
        ...
        items.add(HomePageList("أفلام جديدة", newMovies))
        
        // مسلسلات جديدة
        val newSeries = Jsoup.connect("$mainUrl/series").get()
            .select(".series-item").map { element ->
                val title = element.select(".title").text()
                val poster = element.select("img").attr("src")
                val url = element.select("a").attr("href")
                val year = element.select(".year").text().toIntOrNull()
        ...
        items.add(HomePageList("مسلسلات جديدة", newSeries))
        
        return HomePageResponse(items)
    }

    override suspend fun search(query: String): List<SearchResponse> {
        val doc = Jsoup.connect("$mainUrl/search?s=$query").get()
    ...
    override suspend fun loadLinks(
        ...
        doc.select(".watch-link").forEach { element ->
